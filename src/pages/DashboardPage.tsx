...
export const DashboardPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {profile?.trialEndsAt && profile?.tier === 'creator' && (
        <motion.div 
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-12 p-6 bg-gradient-to-r from-[#EFBB76]/20 to-[#EFBB76]/5 border border-[#EFBB76]/30 rounded-[2rem] flex items-center justify-between gap-6"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#EFBB76] rounded-full flex items-center justify-center shadow-lg">
              <Clock className="w-6 h-6 text-black" />
            </div>
            <div>
              <h3 className="text-lg font-serif font-bold text-black">Trial Period Active</h3>
              <p className="text-[10px] font-bold uppercase tracking-widest text-black/60">
                You have {getDaysRemaining(profile.trialEndsAt)} days remaining in your Creator trial.
              </p>
            </div>
          </div>
          <button 
            onClick={() => navigate('/tiers')}
            className="px-8 py-3 bg-black text-white rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-black/80 transition-all shadow-md"
          >
